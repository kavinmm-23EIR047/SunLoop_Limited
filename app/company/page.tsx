  ...
  return (
    <main className="font-sans bg-[#FAFAF3]">
      ...
      <section className="container max-w-5xl py-20 md:py-28">
        <div className="grid gap-6 md:grid-cols-2">
          <Reveal className="dark-panel p-8 text-white rounded-3xl relative overflow-hidden bg-brand-ink">
            <div className="relative z-10 space-y-4">
              ...
              <h2 className="text-2xl font-bold leading-snug">To power homes, businesses, and industries with clean, optimized, renewable energy — delivered through advanced Solar, Storage, and EV Charging systems built for the long term.</h2>
            </div>
          </Reveal>
          ...
          <Reveal className="panel p-8 rounded-3xl bg-white shadow-card border border-black/10">
            <div className="space-y-4">
              ...
              <h2 className="text-2xl font-bold text-brand-ink leading-snug">To design, deploy, and manage intelligent renewable energy solutions that reduce electricity costs and carbon footprint simultaneously, while ensuring long-term energy security through world-class engineering and support.</h2>
            </div>
          </Reveal>
        </div>
        ...
        <div className="mt-8 grid gap-3 md:grid-cols-2 text-sm font-medium text-brand-ink">
          {[
            ...
            <Reveal key={x} className="flex items-start gap-3 border-b border-black/10 py-3.5">
              <Check className="h-4 w-4 text-brand-primary shrink-0 mt-0.5" aria-hidden="true" />
              <span>{x}</span>
            </Reveal>
          ))}
        </div>
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {values.map(([title, desc, Icon]) => (
              <Reveal className="panel p-6 rounded-2xl bg-white shadow-xs border border-black/10 hover:shadow-card transition" key={title}>
                ...
                <p className="mt-2 text-sm leading-relaxed text-brand-slate font-normal">{desc}</p>
              </Reveal>
            ))}
          </div>
        </div>
