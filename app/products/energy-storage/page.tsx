...
export default function EnergyStoragePage() {
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF5] pt-16 pb-16 font-sans">
      ...
      {/* CATALOG SECTION WITH RESIDENTIAL & COMMERCIAL FILTER TABS */}
      <section className="py-14 md:py-20">
        <div className="container max-w-5xl mx-auto px-4 space-y-12">
          <Reveal className="text-center max-w-2xl mx-auto space-y-3">
            <span className="eyebrow inline-block">STORAGE PORTFOLIO</span>
            <h2 className="text-2xl md:text-3xl font-bold text-brand-ink">
              Residential, Commercial &amp; Utility Series
            </h2>
            <p className="text-xs md:text-sm text-brand-slate font-normal">
              Categorized lithium battery storage systems tailored for home, business, and utility grid applications.
            </p>

            {/* FILTER BUTTONS: RESIDENTIAL & COMMERCIAL */}
            <div className="pt-4 flex justify-center items-center gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                  selectedFilter === 'all'
                    ...
                    : 'bg-white border border-black/10 text-brand-slate hover:bg-black/5'
                }`}
              >
                All Battery Packs ({productsList.length})
              </button>
              <button
              ...
              >
                <Home className="h-3.5 w-3.5" /> Residential ({productsList.filter(p=>p && p.keyword==='residential').length})
              </button>
              <button
              ...
              >
                <Building2 className="h-3.5 w-3.5" /> Commercial &amp; Utility ({productsList.filter(p=>p && p.keyword==='commercial').length})
              </button>
            </div>
          </Reveal>
          ...
          <div className="space-y-16">
            {filteredProducts?.map((prod, index) => {
              ...
              return (
                <div
                  key={prod.id}
                  className="grid gap-10 md:grid-cols-2 items-center py-6 border-b border-black/[0.06] last:border-0"
                >
                    ...
                    <div className="space-y-1.5">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`rounded-md px-2.5 py-0.5 text-[10px] font-bold uppercase ${
                          prod.keyword === 'residential' ? 'bg-orange-100 text-[#E86526]' : 'bg-blue-100 text-blue-700'
                        }`}>
                          {prod.categoryLabel}
                        </span>
                        <span className="rounded-md bg-neutral-100 border border-neutral-200 px-2.5 py-0.5 text-[10px] font-bold text-neutral-700 uppercase">
                          {prod.series}
                        </span>
                      </div>
