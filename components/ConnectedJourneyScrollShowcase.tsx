...
const stages: StageCardData[] = [
  {
    ...
    eyebrow: 'Phase One',
    title: 'Start Today',
    desc: 'Purchase any single solution that fits your current needs — Solar PV, Battery ESS, or EV Fast Chargers.',
  ...
  },
  {
    ...
    desc: 'Expand with energy storage when backup power, load shaving, or zero-outage security is required.',
...
export default function ConnectedJourneyScrollShowcase() {
  return (
    <section className="relative overflow-clip bg-white border-y border-black/5 py-16 md:py-32">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center select-none opacity-[0.03]" aria-hidden="true">
      ...
      <div className="container relative z-10 mx-auto max-w-6xl px-4">
        <div className="mx-auto mb-16 max-w-3xl space-y-4 text-center md:mb-24">
          ...
          <h2 className="text-3xl text-brand-ink sm:text-4xl md:text-5xl font-bold leading-tight">One Connected Energy Ecosystem</h2>
          <p className="mx-auto max-w-2xl text-sm leading-relaxed text-brand-slate md:text-base">
            We don't sell isolated products. We build one connected ecosystem that scales with you — start with a single system today, and expand into a fully integrated Solar + Storage + EV microgrid whenever you're ready.
          </p>
        </div>
              ...
              >
                ...
                >
                  <div>
                    ...
                    <span className="mb-3 block text-xs font-semibold tracking-wide text-brand-primary">{stage.eyebrow}</span>
                    <h3 className={`mb-4 text-2xl sm:text-3xl font-semibold leading-tight tracking-tight ${stage.isDark ? 'text-white' : 'text-brand-ink'}`}>{stage.title}</h3>
                    <p className={`text-sm sm:text-base leading-relaxed ${stage.isDark ? 'text-white/80' : 'text-brand-slate'}`}>{stage.desc}</p>
                  </div>
                  <div className={`mt-10 border-t pt-5 text-xs sm:text-sm font-semibold tracking-wide leading-relaxed text-brand-primary ${stage.isDark ? 'border-white/10' : 'border-black/10'}`}>
                    {stage.foot}
                  </div>
                </article>
              </div>
